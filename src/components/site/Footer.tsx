...
import { Glasses, Twitter, Instagram, Linkedin, Github, Send } from "lucide-react";
...
export const Footer = () => {
  ...
  const subscribe = (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    toast({ title: "Subscribed!", description: "Thanks for joining the EchoSee newsletter." });
    setEmail("");
  };
  ...
  return (
    <footer className="relative pt-20 pb-10 border-t border-border mt-10">
      <div className="container-pad grid md:grid-cols-4 gap-10">
        <div className="md:col-span-2">
          <a href="#home" className="flex items-center gap-2">
            ...
            <span className="font-display font-bold text-xl">Echo<span className="gradient-text">See</span></span>
          </a>
          <p className="mt-4 text-muted-foreground max-w-sm">
            See what you cannot hear. Real-time AR subtitles for everyone.
          </p>
          <form onSubmit={subscribe} className="mt-6 max-w-md">
            <label htmlFor="newsletter" className="text-sm font-medium mb-2 block">Join the newsletter</label>
            <div className={`flex items-center gap-2 glass-card p-1 transition-all ${focus ? "border-primary shadow-glow" : ""}`}>
              <input
                id="newsletter"
                type="email"
                value={email}
              ...
              />
              <Button type="submit" variant="hero" size="sm" className="rounded-md">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <li><a href="#preorder" className="hover:text-primary transition">Pre-order</a></li>
          </ul>
        </div>
        <div>
          ...
          <ul className="space-y-2 text-sm text-muted-foreground">
            ...
            <li><a href="#partnerships" className="hover:text-primary transition">Partnerships</a></li>
      ...
      <div className="container-pad mt-12 pt-6 border-t border-border flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-muted-foreground">
